            ...
            <div class="flex items-start justify-between gap-4">
                ...
                <div class="flex items-center gap-3">
                    <?php if(auth()->check() && auth()->user()->role === 'admin'): ?>
                        <a href="<?php echo e(route('admin.services.index')); ?>" class="inline-flex shrink-0 items-center gap-2 rounded-full border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50">
                            Services
                        </a>
                    <?php endif; ?>
...
<script>
    const pageServices = <?php echo json_encode($pageServices, 15, 512) ?>;//envoyer un var php/laravel => js
    const cacheLieux = {};
    ...
    const estConnecte = <?php echo json_encode(auth()->check(), 15, 512) ?>;
    const estAdmin = <?php echo json_encode(auth()->check() && auth()->user()->role === 'admin', 15, 512) ?>;
    const idUtilisateur = <?php echo json_encode(auth()->id(), 15, 512) ?>;
    const urlConnexion = <?php echo json_encode(route('login'), 15, 512) ?>;
    const urlInscription = <?php echo json_encode(route('register'), 15, 512) ?>;
...
<?php $__env->stopPush(); ?>
<?php endif; ?>
<?php echo $__env->make('partials.header', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Apache24\htdocs\Urbanoo\resources\views/welcome.blade.php ENDPATH**/ ?>